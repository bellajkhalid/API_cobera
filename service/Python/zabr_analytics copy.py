...
# Define paths
xsigma_path = r"C:\dev\build_ninja_avx2_python_sphinx\lib\python3.11\site-packages"
python_service_path = r"C:\Users\bella\OneDrive\Desktop\API_Xsigma\Backend_Xsigma\service\Python"
module_path = os.path.join(xsigma_path, "xsigmamodules")
common_path = os.path.join(python_service_path, "common")

# Check and add paths
if os.path.exists(xsigma_path):
    print(f"xsigmamodules path exists: {xsigma_path}")
    sys.path.append(xsigma_path)
else:
    print(f"ERROR: xsigmamodules path does not exist: {xsigma_path}")

if os.path.exists(python_service_path):
    print(f"Python service path exists: {python_service_path}")
    sys.path.append(python_service_path)
else:
    print(f"ERROR: Python service path does not exist: {python_service_path}")

# Check if module directories exist
print(f"Module path exists: {os.path.exists(module_path)}")
print(f"Common module path exists: {os.path.exists(common_path)}")
...
try:
    import common.sabrHelper
    print("Common module import successful")
except ImportError as e:
    print(f"Common module import error: {e}")

# Continue with the rest of the script only if imports were successful
try:
    import json
    import numpy as np
    ...
    if __name__ == "__main__":
        try:
            ...
            if len(sys.argv) < 2:
                print("No parameters provided. Using default classical model parameters.")
                params = {
                ...
                # Read parameters from command line (passed as JSON string)
